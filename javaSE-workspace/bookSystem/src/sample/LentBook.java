...
public class LentBook implements Initializable {
    ...
    public void click_btn_search() throws IOException {
        ...
        for (int i = 0; i <num_list.size(); i++) {
            if(book_num.equals(num_list.get(i))&&(isOut_list.get(i).equals("在馆"))){
                String newInfo=name_list.get(i)+","+num_list.get(i)+","+type_list.get(i)+","+
                        press_list.get(i)+","+writer_list.get(i)+","+"借出"+","+user_num+","+
                        (Integer.parseInt(fre_list.get(i))+1);
                write_list.add(newInfo);
                show_list.add(new bookAdmin(book_list.get(i)));
                Alert alert = new Alert(Alert.AlertType.INFORMATION);
                alert.setTitle("成功");
                alert.setHeaderText("借书成功");
                alert.setContentText("已成功借出"+name_list.get(i));
        ...
        for (int i = 0; i <user_list.size(); i++) {
            ...
            {
                String newInfo;
                if(!userbook_list.get(i).equals("无")) {
                    newInfo = user_num + "," + password_list.get(i) + "," + userbook_list.get(i) + "+" + book_num
                            + "," + email_list.get(i);
                }
                else{
                    newInfo = user_num + "," + password_list.get(i) + "," + book_num
                            + "," + email_list.get(i);
                }
